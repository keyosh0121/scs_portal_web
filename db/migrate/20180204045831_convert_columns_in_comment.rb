class ConvertColumnsInComment < ActiveRecord::Migration[5.1]
  def change
    ...
    Comment.all.each_with_index {|com,i| com.event_id = comment_ids[i] }
    ...
    Comment.all.each_with_index {|com,i| com.content_id = content_ids[i] }

    #Change all the 'sender' attributes to 'sender_id'
    sender_ids = Comment.all.map{|com| User.find_by(name: com.sender)}.map(&:id)
    Comment.all.each_with_index {|com,i| com.sender_id = sender_ids[i] }
